...
# 钉钉工作通知
NOTIFY_SEND_API = 'https://prod.common-notify.sgt.sg2.api/notify/dingapp/send'
# 机器人通知
ROBOT_API = "https://oapi.dingtalk.com/robot/send"
...
class Notify:
    ...
    def __post_url(self, url, headers, data):
        try:
            print ("url: %s, headers: %s, data: %s" % (url, str(headers), str(data)))
            session = requests.session()
            session.keep_alive = False
            resp = session.post(url, headers=headers, json=data, verify=False, timeout=1200)
            print(resp.text)
            session.close()
            print ("notify success. url: %s" % url)
        except Exception as e:
            print ("require url: %s error, cause: %s." % (url, str(e)))
